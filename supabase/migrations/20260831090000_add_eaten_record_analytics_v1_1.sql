-- PREPARED FOR ANALYTICS V1.1. NOT EXECUTED AGAINST SUPABASE.
BEGIN;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;

DO $eaten_analytics_preflight$
DECLARE
  v_constraint_definition text;
  v_event_names text[];
  v_analytics_count bigint;
  v_analytics_fingerprint text;
  v_restaurants_fingerprint text;
  v_menus_fingerprint text;
  v_weekly_fingerprint text;
  v_log_public_execute boolean;
  v_dashboard_public_execute boolean;
BEGIN
  IF to_regclass('public.analytics_events') IS NULL
    ...
    RAISE EXCEPTION 'Eaten Analytics preflight failed: required tables are missing';
  END IF;
  ...
  IF v_event_names IS DISTINCT FROM ARRAY['map_open', 'menu_card_open', 'recommendation_error', 'recommendation_refresh', 'recommendation_shown', 'session_start', 'share_recommendation']::text[] THEN
    RAISE EXCEPTION 'Eaten Analytics preflight failed: current event allowlist drifted';
  END IF;
  ...
  IF (
    ...
    'public.log_analytics_event(uuid, text, timestamptz, uuid, uuid, text, text, smallint, text, text, smallint, text, text)'
  ) IS NULL THEN
    RAISE EXCEPTION 'Eaten Analytics preflight failed: ingestion RPC signature drifted';
  END IF;
  ...
  ) <> 1 OR to_regprocedure('public.get_admin_analytics_dashboard()') IS NULL THEN
    RAISE EXCEPTION 'Eaten Analytics preflight failed: dashboard RPC signature drifted';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc AS procedure_row
    WHERE procedure_row.oid = 'public.log_analytics_event(uuid, text, timestamptz, uuid, uuid, text, text, smallint, text, text, smallint, text, text)'::regprocedure
      AND procedure_row.prorettype = 'boolean'::regtype
      AND procedure_row.prosecdef
      AND procedure_row.proconfig @> ARRAY['search_path=pg_catalog']::text[]
      AND pg_catalog.pg_get_functiondef(procedure_row.oid) NOT LIKE '%eaten_record_added%'
  ) OR NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc AS procedure_row
    WHERE procedure_row.oid = 'public.get_admin_analytics_dashboard()'::regprocedure
      AND procedure_row.prorettype = 'jsonb'::regtype
      AND procedure_row.prosecdef
      AND procedure_row.proconfig @> ARRAY['search_path=pg_catalog']::text[]
      AND pg_catalog.pg_get_functiondef(procedure_row.oid) LIKE '%public.is_admin()%'
      AND pg_catalog.pg_get_functiondef(procedure_row.oid) NOT LIKE '%eaten_records%'
  ) THEN
    RAISE EXCEPTION 'Eaten Analytics preflight failed: function contract drifted';
  END IF;

  IF NOT (SELECT relrowsecurity FROM pg_catalog.pg_class WHERE oid = 'public.analytics_events'::regclass)
    OR (SELECT relforcerowsecurity FROM pg_catalog.pg_class WHERE oid = 'public.analytics_events'::regclass)
    OR EXISTS (
    ...
    RAISE EXCEPTION 'Eaten Analytics safety check failed: raw table security drifted';
  END IF;
  ...
  SELECT EXISTS (
    ...
    WHERE procedure_row.oid = 'public.get_admin_analytics_dashboard()'::regprocedure
      AND acl_row.grantee = 0
      AND acl_row.privilege_type = 'EXECUTE'
  ) INTO v_dashboard_public_execute;

  IF v_log_public_execute
    OR NOT has_function_privilege('anon', 'public.log_analytics_event(uuid, text, timestamptz, uuid, uuid, text, text, smallint, text, text, smallint, text, text)', 'EXECUTE')
    OR NOT has_function_privilege('authenticated', 'public.log_analytics_event(uuid, text, timestamptz, uuid, uuid, text, text, smallint, text, text, smallint, text, text)', 'EXECUTE')
    OR v_dashboard_public_execute
    OR has_function_privilege('anon', 'public.get_admin_analytics_dashboard()', 'EXECUTE')
    OR NOT has_function_privilege('authenticated', 'public.get_admin_analytics_dashboard()', 'EXECUTE') THEN
    RAISE EXCEPTION 'Eaten Analytics safety check failed: function grants drifted';
  END IF;

  SELECT count(*), md5(COALESCE(jsonb_agg(to_jsonb(row_value) ORDER BY row_value.event_id), '[]'::jsonb)::text)
  INTO v_analytics_count, v_analytics_fingerprint
  FROM public.analytics_events AS row_value;
  ...
  INTO v_weekly_fingerprint
  FROM public.restaurant_weekly_hours AS row_value;

  PERFORM set_config('mukjji.eaten_analytics_count', v_analytics_count::text, true);
  PERFORM set_config('mukjji.eaten_analytics_fingerprint', v_analytics_fingerprint, true);
  PERFORM set_config('mukjji.eaten_restaurants_fingerprint', v_restaurants_fingerprint, true);
  PERFORM set_config('mukjji.eaten_menus_fingerprint', v_menus_fingerprint, true);
  PERFORM set_config('mukjji.eaten_weekly_fingerprint', v_weekly_fingerprint, true);
END;
$eaten_analytics_preflight$;
...
BEGIN
  ...
  ELSE
    ...
    ELSIF p_event_name = 'eaten_record_added' THEN
      IF p_restaurant_id IS NULL
        OR p_menu_id IS NULL
        OR p_recommendation_id IS NOT NULL
        OR p_position IS NOT NULL
        ...
        OR p_item_count IS NOT NULL
        OR p_share_method IS NOT NULL THEN
        RAISE EXCEPTION 'Analytics event rejected: invalid eaten_record_added fields'
          USING ERRCODE = '22023';
      END IF;
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'today', jsonb_build_object(
      ...
      'shares', today.shares,
      'errors', today.errors,
      'eaten_records', today.eaten_records
    ),
    ...
    'last_7_days', jsonb_build_object(
      ...
      'eaten_records', seven_days.eaten_records
    ),
...
BEGIN
  ...
  IF v_event_names IS DISTINCT FROM ARRAY['eaten_record_added', 'map_open', 'menu_card_open', 'recommendation_error', 'recommendation_refresh', 'recommendation_shown', 'session_start', 'share_recommendation']::text[] THEN
    RAISE EXCEPTION 'Eaten Analytics postcheck failed: event allowlist mismatch';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE procedure_row.oid = 'public.log_analytics_event(uuid, text, timestamptz, uuid, uuid, text, text, smallint, text, text, smallint, text, text)'::regprocedure
      ...
      AND procedure_row.prosecdef
      AND procedure_row.proconfig @> ARRAY['search_path=pg_catalog']::text[]
      AND pg_catalog.pg_get_functiondef(procedure_row.oid) LIKE '%invalid eaten_record_added fields%'
  ) OR NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc AS procedure_row
    WHERE procedure_row.oid = 'public.get_admin_analytics_dashboard()'::regprocedure
      AND procedure_row.prorettype = 'jsonb'::regtype
      AND procedure_row.prosecdef
      AND procedure_row.proconfig @> ARRAY['search_path=pg_catalog']::text[]
      AND pg_catalog.pg_get_functiondef(procedure_row.oid) LIKE '%eaten_records%'
      AND pg_catalog.pg_get_functiondef(procedure_row.oid) LIKE '%internal_test_sessions AS MATERIALIZED%'
  ) THEN
    RAISE EXCEPTION 'Eaten Analytics postcheck failed: function contract mismatch';
  END IF;
  ...
  IF v_analytics_count::text <> current_setting('mukjji.eaten_analytics_count')
    OR v_analytics_fingerprint <> current_setting('mukjji.eaten_analytics_fingerprint')
    OR v_restaurants_fingerprint <> current_setting('mukjji.eaten_restaurants_fingerprint')
    OR v_menus_fingerprint <> current_setting('mukjji.eaten_menus_fingerprint')
    OR v_weekly_fingerprint <> current_setting('mukjji.eaten_weekly_fingerprint') THEN
    RAISE EXCEPTION 'Eaten Analytics postcheck failed: existing data changed';
  END IF;
END;
$eaten_analytics_postcheck$;

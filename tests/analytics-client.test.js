...
function migrationFunctionParameters() {
  const migration = fs.readFileSync(
    path.join(root, "supabase", "migrations", "20260823160000_add_analytics_acquisition_source.sql"),
    "utf8",
  );
...
async function run() {
  ...
  assert.equal(getAcquisitionSource({ search: "" }), "direct");
  assert.equal(getAcquisitionSource({ search: "?src=Everytime" }), "everytime");
  assert.equal(getAcquisitionSource({ search: "?src=" }), "other");
  assert.equal(getAcquisitionSource({ search: "?src=unknown-raw-value" }), "other");
  assert.equal(getAcquisitionSource({ get search() { throw new Error("blocked"); } }), "direct");
  assert.equal(
    isAnalyticsRuntimeEnabled({ hostname: "changwon-food-app.vercel.app" }),
    true,
    "the exact production hostname enables collection when the release switch is on",
  );
  assert.equal(isAnalyticsRuntimeEnabled({ hostname: "localhost" }), false);
  assert.equal(isAnalyticsRuntimeEnabled({ hostname: "127.0.0.1" }), false);
  assert.equal(
    isAnalyticsRuntimeEnabled({ hostname: "changwon-food-app-git-feature-analytics-example.vercel.app" }),
    false,
    "Vercel Preview hostnames must remain disabled",
  );
  assert.equal(isAnalyticsRuntimeEnabled({ hostname: "unknown.example" }), false);
  assert.equal(isAnalyticsRuntimeEnabled({ hostname: "" }), false);
  assert.equal(isAnalyticsRuntimeEnabled(undefined), false);
  assert.equal(isAnalyticsRuntimeEnabled({
    get hostname() {
      throw new Error("blocked");
    },
  }), false, "location access failures must fail closed");
  assert.equal(
    isAnalyticsRuntimeEnabled({ hostname: "changwon-food-app.vercel.app" }, false),
    false,
    "the collection switch still disables the exact production host",
  );
  assert.doesNotMatch(analyticsSource, /\*\.vercel\.app|includes\(["']vercel["']\)|endsWith\(["']\.vercel\.app["']\)/);
  ...
  assert.match(
    appSource,
    /typeof window\.MukjjiAnalytics\?\.createAnalyticsClient === "function"\s*&&\s*typeof window\.MukjjiAnalytics\?\.isAnalyticsRuntimeEnabled === "function"/,
  );
  assert.match(appSource, /enabled:\s*window\.MukjjiAnalytics\.isAnalyticsRuntimeEnabled\(\)/);
  ...
  const disabledClient = createAnalyticsClient({
    enabled: isAnalyticsRuntimeEnabled({ hostname: "localhost" }),
  ...
  assert.equal(disabledLogicalEvents.find((event) => event.eventName === "session_start").acquisitionSource, "poster_qr");
  assert.ok(disabledStorage.getItem(SESSION_ID_KEY));
  assert.ok(disabledStorage.getItem(LAST_ACTIVITY_KEY));

  let productionMockRpcCalls = 0;
  const productionMockClient = createAnalyticsClient({
    enabled: isAnalyticsRuntimeEnabled({ hostname: "changwon-food-app.vercel.app" }),
    ...
    getSupabaseClient: async () => ({
      rpc(name) {
        productionMockRpcCalls += 1;
        assert.equal(name, RPC_FUNCTION_NAME);
        return { data: true, error: null };
      },
    }),
  });
  assert.equal(await productionMockClient.initialize(), true);
  assert.equal(productionMockRpcCalls, 1, "the exact production host can enter the mocked RPC path");
  ...
  assert.ok(sessionEvents.filter((event) => event.eventName === "session_start").every((event) => event.acquisitionSource === "kakao"));
  ...
  console.log("analytics client: production-host gate, sessions, retry, recommendation lifecycle, contract, privacy passed");
}
